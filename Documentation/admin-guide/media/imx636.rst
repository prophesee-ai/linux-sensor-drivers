...
an EVS, each pixel produces data asynchrously, as a result, there is no
repeated data structure, such as the frame in usual, frame-based, vision
sensors, and there is no fixed-rate output. Instead, there is a stream of
independent pixel events (usually the pixel coordinates, the polarity of the
luminance change -increasing or decreasing-, and the timestamp of the event,
to reject the acquisition pipeline jitter when processing the data.
...
The hardware description (in the devicetree) allows a
``imx636,fixed-packet-size`` quirk, to tell the driver to always wait for
packet completion before sending it. This breaks the latency guarantees
at packet level, but may be necessary to accommodate the CSI-2 receiver.
When this option is set, a packet is sent in either of two conditions:

- The packet is full
- The end of the frame period is reached. In this case, a long packet
  with a size smaller than the maximum is sent before the small packet
  signaling the end of frame
...
- lanes at 300, 400, 600 or 750MHz
- data output in EVT 2.1 (with limitations) or EVT 3.0
- Square region of interest (through crop selection)
- some test patterns
...
integrated upstream, requiring a rebuild of the userspace with the new
...
The media bus pixel codes describe image formats as flowing over physical busses.
The width advertised by the driver on media pads indicates the size of the
packets sent on the MIPI CSI-2 link, as some receivers need it. It usually
differs from the sensor pixel matrix width. Then, the height is not directly
managed by the IMX636. It is just the number of packets in a frame, and since
this number varies, the height given by the media-ctl pad is just an upper bound
of that number, so that the actual information never overflows a memory buffer
allocated by a receiver.
